...
interface ChapterWritingDecisionAgentConfig {
  projectId: string;
  draftId: string;
 ...
 */
export class ChapterWritingDecisionAgent extends HttpAgent {
  private draftId: string;
  private decision: UserDecision | null = null;

  constructor(config: ChapterWritingDecisionAgentConfig) {
    const baseUrl =
      config.baseUrl || process.env.NEXT_PUBLIC_API_URL || "/api/v1";
    super({
      url: `${baseUrl}/generate/chapter-writing/${config.projectId}/decide`,
